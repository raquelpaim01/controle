...
if (isset($_GET['acao']) && $_GET['acao'] === "delete") {
     $res = pg_query($conexao,"delete from fornecedor where id = ".$_GET['id']);
     
     if(!$res){
         echo '<p>'.pg_last_error($conexao).'</p>';
         die( '<a href="consulta_fornecedor.php">Retornar para listagem</a>');
...
if( $pessoa ==='fisica'){
    $sql = "INSERT INTO FORNECEDOR (razao_social,email,cpf, telefone, rua, numero, bairro, cep, cidade, estado)VALUES(";
    $sql .= "'$converte',";
    $sql  .="'$email',";
    $sql .="'$informeonumero',";
    $sql .= "'$telefone',";
    ...
    $sql = "INSERT INTO FORNECEDOR ( razao_social,email,telefone,cnpj, rua, numero, bairro, cep, cidade, estado)VALUES(";
...
$result = pg_query($conexao,$sql);
    if(!$result){
        echo '<p>'.pg_last_error($conexao).'</p>';
        die( '<a href="fornecedor.html">Retornar </a>');
    }
