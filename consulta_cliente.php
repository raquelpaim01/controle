...
$conexao = pg_connect("host=$servidor port=$porta dbname=$banco user=$usuario password=$senha");
   ...
   $informe = $_POST["informe"];
         ...
         if ($pessoa=='nome'){
             $sql="SELECT * FROM vw_cliente where razao_social ilike $1 order by razao_social";
             ...
             while ($tbl = pg_fetch_array($result)){
                    ...
                    echo "<td><A href=\"edicao_cliente.php?acao=update&id=".$tbl['id']."\">";
                    echo "(Editar)</A></td>";
                    echo "<td><A href=\"dados_cliente.php?acao=delete&id=".$tbl['id']."\">";
                   echo "(Excluir)</A></td>";
        ...
        $sql="SELECT * FROM vw_cliente where cpf = $1 ";
        $result = pg_query_params($conexao,$sql,array(pg_escape_string($_POST['informe'])));
        while ($tbl = pg_fetch_array($result)){
       echo "<td> ".$tbl['id']. "</td>";
       echo "<td> ".$tbl['razao_social']. "</td>";
      ...
      echo "(Excluir)</A></td>";
       echo "</tr>";
        ...
        $sql="SELECT * FROM vw_cliente where cnpj = $1";
        $result = pg_query_params($conexao,$sql,array(pg_escape_string($_POST['informe'])));
        while ($tbl = pg_fetch_array($result)){
       echo "<td> ".$tbl['id']. "</td>";
       echo "<td> ".$tbl['razao_social']. "</td>";
      ...
      echo "(Excluir)</A></td>";
       echo "</tr>";
        ...
        die( '<a href="cliente.php">Retornar cadastro de cliente</a>');
